...
(async () => {
	try {
		...
		console.log(wsClient._schemas.transactionsAssets[4].schema.properties.unlockObjects);
		...
		const unvote = await wsClient.transaction.create(unvoteTrs, accounts.newAccount.passphrase);
		const unvoteJSON = wsClient.transaction.toJSON(unvote);
		console.log(JSON.stringify(unvoteJSON, null, 2));

		// Unlock funds from unvote
		const unlockTrs = {
			moduleID: 5,
			assetID: 2,
			senderPublicKey: accounts.newAccount.publicKey,
		...
		const unlock = await wsClient.transaction.create(unlockTrs, accounts.newAccount.passphrase);
		const unlockJSON = wsClient.transaction.toJSON(unlock);
		console.log(JSON.stringify(unlockJSON, null, 2));
